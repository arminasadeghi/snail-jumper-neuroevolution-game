...
class Player(pygame.sprite.Sprite):
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        """
        input_vector=self.make_input_vector(obstacles, player_x, player_y)
        result=self.nn.forward(input_vector)[0]
    ...
    def make_input_vector(self,obstacles, player_x, player_y,):
        #man bias too hamoon nn dadam age lazem shod bayad inja bedam
        input_vector=[]
        if(len(obstacles)>1):

            input_vector.append(float(list(obstacles[0].values())[0]))
            input_vector.append(float(list(obstacles[0].values())[1]))
            input_vector.append(float(list(obstacles[1].values())[0]))
            input_vector.append(float(list(obstacles[1].values())[1]))
            input_vector.append(player_x)
            input_vector.append(player_y)
